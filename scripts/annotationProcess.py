...
class Labels:
    ...
    def dframes(self):
        ...
        celdas_vertices_end = arcpy.FeatureVerticesToPointService.FeatureVerticesToPointService(celdas_flayer,
                                                                                                'END')  # USE SERVICE
        ...
        self.dframe_vertice_final_edad = pd.DataFrame(
            shapefile_to_dataframe_04)  # DATA FRAME DE VERTICES FINALES DE EDADES
    ...
    def add_codhoja_annotation(self):
        ...
        with arcpy.da.UpdateCursor(self.glb.path, [self.gpt.codhoja, self.glb.hoja, self.glb.cuadrante],
    ...
    def labelHead(self):
        ...
        centroide_head = self.dframe_centroide[(self.dframe_centroide["SHAPE@Y"] <= y["ymax"]) &
                                               (self.dframe_centroide["SHAPE@Y"] >= y["ymax"] - 550)].sort_values(
            ["SHAPE@X"])
        ...
        for i, v in centroide_head[:4].iterrows():
            itemFeatures = [{"attributes": {self.gpt.nombre: self.label.head_age_label[i + 1],
                                            self.gpt.estilo: self.label.headClass["ages"], self.gpt.hoja: self.hoja100,
                                            self.gpt.cuadrante: self.cuadrante, self.gpt.codhoja: self.hoja},
        ...
        for i, v in centroide_head[4:].iterrows():
            itemFeatures = [{"attributes": {self.gpt.nombre: self.label.head_column_label[types[i - 4]],
                                            self.gpt.estilo: self.label.headClass["colum"], self.gpt.hoja: self.hoja100,
                                            self.gpt.cuadrante: self.cuadrante,
    ...
    def get_edades_label(self, edad):
        ...
        result = list(dframe_edad[self.tabla_edades.nombre]) if edad != "edad" else list(
            dframe_edad[self.tabla_edades.edad_fin])#ANTERIOR edad_ini
        return result
    ...
    def labelAgeNum(self):
        ...
        vertices_edades = self.dframe_vertice_final_edad[(self.dframe_vertice_final_edad["SHAPE@Y"] < y["ymax"] - 650) &
                                                         ...
                                                         (self.dframe_vertice_final_edad[
                                                              "SHAPE@X"] > xmin)].sort_values(["SHAPE@Y"])
        ...
        for i, v in vertices_edades.iterrows():
            itemFeatures = [{"attributes": {self.gpt.nombre: edades[i],
                                            self.gpt.estilo: self.label.laterClass["edad"], self.gpt.hoja: self.hoja100,
                                            self.gpt.cuadrante: self.cuadrante,
                                            self.gpt.codhoja: self.hoja},
                             "geometry": {"x": float(v["SHAPE@X"]), "y": float(v["SHAPE@Y"])}}]
    ...
    def agregar_valores_array(self, dframe, componente, x, oper=None, xlist=None):
        contador = 0
        # if componente == "conjunto_volcanico":
        #     dframe = dframe
        for i, v in dframe.iteritems():
            if componente == "miembro":
                i = i[1]
            if oper:
                txt = "|".join(i.split(" ")) if oper == 2 else i
            elif componente == "conjunto_volcanico":
                txt = i
                x = xlist[contador]
                contador = contador + 1
            else:
            ...
            itemFeatures = [{"attributes": {self.gpt.nombre: txt,
                                            self.gpt.estilo: self.label.annotationClass[componente],
                                            self.gpt.hoja: self.hoja100, self.gpt.cuadrante: self.cuadrante,
                                            self.gpt.codhoja: self.hoja},
    ...
    def componentes_unidades_litoest_label(self, x, estilo, componente):
        if componente == 'MIEMBRO':
            query_data_frame = \
                self.dframe_gpo_leyenda[self.dframe_gpo_leyenda[componente] != "None"].groupby(
                    ['FORMACION', componente])[
                    "SHAPE@Y"].mean()
        else:
            query_data_frame = \
                self.dframe_gpo_leyenda[self.dframe_gpo_leyenda[componente] != "None"].groupby(componente)[
                    "SHAPE@Y"].mean()
    ...
    def formacion_unidades_litoest_label(self, x, oper):
        ...
        query_data_frame = self.dframe_gpo_leyenda[(self.dframe_gpo_leyenda[self.gpo.formacion] != "None") &
                                                   (operacion[oper](self.dframe_gpo_leyenda[self.gpo.miembro],
                                                                    "None"))].groupby(self.gpo.formacion)[
            "SHAPE@Y"].mean()
    ...
    def unidades_litoestatigraficas_label(self, process):
        ...
        if process:
            xtmp = list(self.dframe_gpo_leyenda[(self.dframe_gpo_leyenda[self.gpo.tipo] == 1)].groupby(self.gpo.grupo)[
                            "SHAPE@X"].min())
    ...
    def conjunto_volcanico_morfoestructuras_volcanicas_label(self, x, xlist=None):
        # query_data_frame = self.dframe_gpo_leyenda[self.dframe_gpo_leyenda[self.gpo.cvolc] != "None"].groupby(self.gpo.cvolc)["SHAPE@Y"].mean()
        query_data_frame = self.dframe_gpo_leyenda[(self.dframe_gpo_leyenda[self.gpo.cvolc] != "None")][
                               (self.dframe_gpo_leyenda[self.gpo.cvolc] != "")].groupby(self.gpo.cvolc)[
                               "SHAPE@Y"].max() + 400  ## Debe ser 225,400

        if query_data_frame.size != 0:
            self.agregar_valores_array(query_data_frame, "conjunto_volcanico", x, xlist=xlist)

    def morfoestructuras_volcanicas_label(self, process):
        add = Annot_morfoestructuras_volcanicas()
        if process:
            xtmp = list(self.dframe_gpo_leyenda[(self.dframe_gpo_leyenda[self.gpo.tipo] == 2)].groupby(self.gpo.cvolc)[
                            "SHAPE@X"].min())
            x_cvolc = min(xtmp) + add.distancia_conjunto_volcanico
            self.conjunto_volcanico_morfoestructuras_volcanicas_label(x_cvolc, xtmp)
    ...
    def componentes_intrusivos_subvolcanicos_label(self, x, estilo, componente):
        query_data_frame = self.dframe_gpo_leyenda[self.dframe_gpo_leyenda[componente] != "None"].groupby(componente)[
            "SHAPE@Y"].mean()
        if query_data_frame.size != 0:
            self.agregar_valores_array(query_data_frame, estilo, x)
    ...
    def intrusivos_subvolcanicos_label(self, process):
        ...
        if process:
            xtmp = list(self.dframe_gpo_leyenda[(self.dframe_gpo_leyenda[self.gpo.tipo] == 3)].groupby(self.gpo.batol)[
                            "SHAPE@X"].min())
    ...
    def descripcion_label(self):
        ...
        for i, v in self.dframe_gpo_leyenda.iterrows():
            ...
            itemFeatures = [{"attributes": {self.gpt.nombre: desc,
                                            self.gpt.estilo: self.label.annotationClass["descripcion"],
                                            self.gpt.hoja: self.hoja100, self.gpt.cuadrante: self.cuadrante,
                                            self.gpt.codhoja: self.hoja},
